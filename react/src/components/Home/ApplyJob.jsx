...
const ApplyJob = () => {
    ...
    const fetchJobSeeker = async (userId) => {
        try {
            const response = await fetch(`http://localhost:8080/jobSeeker/getJobSeekerById/${userId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch("http://localhost:8080/appliedJob/createApplyJob", {
                method: "POST",
                headers: {
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg my-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    <label className="block text-gray-700 font-medium mb-2">Resume URL</label>
                    <input
                        className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
